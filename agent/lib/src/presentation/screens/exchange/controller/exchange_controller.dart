...
import 'package:qunzo_agent/src/presentation/screens/exchange/model/exchange_config_model.dart';
import 'package:qunzo_agent/src/presentation/screens/exchange/model/exchange_wallet_model.dart';
import 'package:qunzo_agent/src/presentation/screens/wallets/model/currencies_model.dart';
...
class ExchangeController extends GetxController {
    ...
    try {
      ...
      if (response.status == Status.completed) {
        ...
        if (fromExchangeWalletsList.isNotEmpty &&
          ...
          fromWalletController.text = fromExchangeWalletsList.first.name ?? "";
          if (toExchangeWalletsList.length > 1) {
            toWallet.value = toExchangeWalletsList[1];
            toWalletController.text = toExchangeWalletsList[1].name ?? "";
          } else {
            toWallet.value = toExchangeWalletsList.first;
            toWalletController.text = toExchangeWalletsList.first.name ?? "";
          }
  ...
  void clearFields() {
    ...
    fromWalletController.clear();
    toWalletController.clear();
  }
}
